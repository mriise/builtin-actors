...
mod util {
    ...
    pub fn verifier_allowance(rt: &MockRuntime) -> DataCap {
        DataCap::from(rt.policy.minimum_verified_deal_size.clone().add(42))
    }

    pub fn client_allowance(rt: &MockRuntime) -> DataCap {
        verifier_allowance(rt).sub(&DataCap::from(BigInt::from(1)))
    }
}
...
mod verifiers {
    ...
    fn add_verifier_enforces_min_size() {
        let (h, mut rt) = new_harness();
        let allowance = DataCap::from(&rt.policy.minimum_verified_deal_size.clone().sub(1));
        expect_abort(
            ExitCode::USR_ILLEGAL_ARGUMENT,
...
mod clients {
    ...
    fn many_verifiers_and_clients() {
        ...
        let allowance_verifier = &allowance_client + (&allowance_client);
    ...
    fn minimum_allowance_ok() {
        ...
        let allowance = DataCap::from(&rt.policy.minimum_verified_deal_size);
        h.add_client(&mut rt, &VERIFIER, &CLIENT, &allowance).unwrap();
        h.check_state(&rt);
    }
    ...
    fn rejects_allowance_below_minimum() {
        ...
        let allowance = DataCap::from(&rt.policy.minimum_verified_deal_size - 1);
        expect_abort(
            ExitCode::USR_ILLEGAL_ARGUMENT,
    ...
    fn rejects_allowance_greater_than_verifier_cap() {
        ...
        let allowance = allowance_verifier.add(&1.into());
        expect_abort(
            ExitCode::USR_ILLEGAL_ARGUMENT,
...
mod datacap {
    ...
    fn consume_values() {
        let (h, mut rt) = new_harness();
        let two = DataCap::from(&rt.policy.minimum_verified_deal_size * 2);
        let one = DataCap::from(&rt.policy.minimum_verified_deal_size);
    ...
    fn consume_exhausted() {
        let (h, mut rt) = new_harness();
        let deal_size = DataCap::from(&rt.policy.minimum_verified_deal_size);
        expect_abort(
    ...
    fn consume_requires_market_actor_caller() {
        ...
        let params = UseBytesParams {
            address: *CLIENT,
            deal_size: DataCap::from(&rt.policy.minimum_verified_deal_size),
        };
        expect_abort(
            ExitCode::USR_FORBIDDEN,
            rt.call::<VerifregActor>(
    ...
    fn consume_requires_minimum_deal_size() {
        let (h, mut rt) = new_harness();
        let deal_size = DataCap::from(&rt.policy.minimum_verified_deal_size - 1);
        expect_abort(
            ExitCode::USR_ILLEGAL_ARGUMENT,
